...
public class SecurityConfig extends  WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception{
		...
		http.authorizeRequests() 					// 認可に関する設定
			.antMatchers("/","/registeruser/form","/registeruser/create").permitAll() // 「/」などのパスはすべてのユーザーに許可
		...
		http.formLogin() 							//ログインに関する設定
			...
			.defaultSuccessUrl("/viewItemList/list", false)	//　第1引数：デフォルトでログイン成功時に遷移させるパス
	...
	/**
	 * 「認証」に関する設定.
	 * 認証ユーザーを取得する「UserDetailsService」の設定や
	 ...
	 * これをしていすることでパスワードの暗号化やマッチ確認する際に
	 * @Autowired
